...
async def fetch_and_upload_recording(
    ...
    """
    Poll Exotel for recording with exponential backoff.
    """
    attempt = 0
    while attempt < max_attempts:
        try:
            ...
            logger.error("recording_poll_error", extra={"interaction_id": interaction_id, "error": str(e)})

        # Exponential backoff
        await asyncio.sleep(base_delay * (2 ** attempt))
        attempt += 1

    logger.error("recording_failed", extra={"interaction_id": interaction_id})
    return None
